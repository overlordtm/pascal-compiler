...
public class LexerTest {
	...
	private File openTestXML(String name) throws IOException {
		...
		if (f.exists()) {
			...
			throw new IOException(String.format("File %s does not exists", name));
		}
	}
	
	private void checkOutput(String[] shouldContain) {
		checkOutputContains(shouldContain);
	}
	
	private void checkOutput(String[] shouldContain, String[] shouldNotContain) {
		if(shouldContain != null) {
			checkOutputContains(shouldContain);
		}
		if(shouldNotContain != null) {
			checkOutputNotContains(shouldNotContain);
		}
	}
	...
	private void checkOutputContains(String[] expected) {
		...
		for(String line : expected) {
			...
			assertTrue(String.format("Could not find line \"%s\"", line), contains);
		}
	}
	...
	private void checkOutputNotContains(String[] expected) {
		...
		for(String line : expected) {
			boolean contains = output.contains(line);
			assertTrue(String.format("Line \"%s\" should not be in output", line), !contains);
		}
	}
	...
	public void testConstantBad() {
		...
		String[] errors = {
				":-o Can not understand ''' (line: 0, column: 0)",
